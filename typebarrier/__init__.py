...
def _convert_list_args_to_kwargs(target: t.Any, value: t.List) -> t.Any:
    ...
    for index, p in enumerate(sig.parameters.values()):
        if p.kind == inspect.Parameter.VAR_KEYWORD:
            ...
            result[p.name] = _convert_value(p.annotation, value[index])
...
def _convert_list(target: t.Any, value: t.List) -> t.Any:
    raise NotImplemented()


def _convert_dictionary(target: t.Any, value: t.Dict) -> t.Any:
    raise NotImplemented()


def _convert_value(target: t.Any, value: t.Any) -> t.Any:
    if isinstance(value, dict):
        return _convert_dictionary(target, value)
    elif isinstance(value, list):
        return _convert_list(target, value)
        ...
        try:
            ...
            raise TypeError(f'can\'t convert a {type(value)} to {target}.')
...
def _typeify_callable(target: t.Callable, d: TwDict) -> TwDict:
    return _convert_dictionary(target, d)
...
def typeify(target: t.Any, d: TwDict) -> TwDict:
    """Given a """
    if callable(target):
        return _typeify_callable(target, d)
    else:
        raise NotImplemented()
